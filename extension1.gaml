...
reflex track_cycles {
   ...
   reflex track_statistics {
    ...
    // Append these counts to their respective lists
...
reflex hunt {
    ...
    // Collect nearby butterflies within the boosted detection range
    ...
    if (!empty(nearby_butterflies)) {
        // Filter butterflies of the most represented color
        list<butterflies> preferred_targets <- nearby_butterflies where (each.color = most_represented_color);

        // If no preferred targets are found, fall back to any nearby butterflies
        list<butterflies> potential_targets <- !empty(preferred_targets) ? preferred_targets : nearby_butterflies;

        // Select a target randomly from potential targets
        butterflies target <- one_of(potential_targets);

        if (target != nil) {
            ask target {
                if (is_alive) {
                    float adjusted_probability <- myself.hunt_probability * instinct_boost * (1 - camouflage_score);
                    if (rnd(1.0) <= adjusted_probability) {
                        // Successful hunt: Reset predator's energy and kill the target
